# Guia Pocket - Vale do Sol 📱

## 📋 Descrição do Projeto
O **Guia Pocket - Vale do Sol** é um aplicativo móvel desenvolvido para divulgar pequenos comércios e serviços do bairro Vale do Sol em Araraquara-SP. O app conecta moradores aos estabelecimentos locais através de uma interface intuitiva e funcional.

## 🎯 Funcionalidades
- 📋 Lista de 6 estabelecimentos locais
- 📞 Ligação direta para os comércios
- 🌐 Acesso a sites e redes sociais
- 🗺️ Localização no Google Maps
- 📤 Compartilhamento de informações
- 🌙 Modo claro e escuro automático
- 🌎 Suporte a português e inglês

## 🏪 Estabelecimentos Incluídos
1. **Abelhinha Uniformes** - Costureira
2. **Supermercado Real 1** - Mercado
3. **Moto Center** - Oficina de motos
4. **BM Barbearia & Tattoo** - Barbearia
5. **Panificadora Vale do Sol** - Padaria
6. **Fabinho Lanches** - Lanchonete

## 📸 Capturas de Tela

### Modo Claro - Português
<p align="center">
  <img src="C:\Users\mcost\OneDrive\Imagens\Capturas de tela\Captura de tela 2025-11-03 234046.png" width="30%" alt="Tela Principal - Modo Claro Português"/>
  <img src=""C:\Users\mcost\OneDrive\Imagens\Capturas de tela\Captura de tela 2025-11-03 234034.png"" width="30%" alt="Tela Detalhes - Modo Claro Português"/>
</p>

### Modo Escuro - Inglês
<p align="center">
  <img src="C:\Users\mcost\OneDrive\Imagens\Capturas de tela\Captura de tela 2025-11-03 234329.png" width="30%" alt="Tela Principal - Modo Escuro Inglês"/>
  <img src="C:\Users\mcost\OneDrive\Imagens\Capturas de tela\Captura de tela 2025-11-03 234319.png" width="30%" alt="Tela Detalhes - Modo Escuro Inglês"/>
</p>

## 🎥 Vídeo Demonstrativo

[**📹 ASSISTIR VÍDEO DEMONSTRATIVO**](https://drive.google.com/file/d/SEU_LINK_DO_DRIVE_AQUI/view?usp=sharing)

*Vídeo de 30 segundos mostrando todas as funcionalidades do aplicativo:*
- Navegação entre telas
- Funcionalidades dos botões (ligar, site, maps, compartilhar)
- Modo claro e escuro
- Troca de idiomas

## 🛠️ Tecnologias Utilizadas
- **Linguagem:** Kotlin
- **IDE:** Android Studio
- **SDK:** Android Tiramissu (API 33)
- **Recursos:** ViewBinding, ListView Personalizado, Intents, Internacionalização

## 📱 Funcionalidades Técnicas
- ✅ ListView com Adapter personalizado
- ✅ Intents explícitas (navegação entre telas)
- ✅ Intents implícitas (ligação, maps, site, compartilhar)
- ✅ Internacionalização (pt-BR e en)
- ✅ Modo Dia/Noite automático
- ✅ ViewBinding em todas as telas

## 👨‍💻 Informações Acadêmicas
**Disciplina:** ARQDMO1 - Dispositivos Móveis 1  
**Professor:** Henrique Galati  
**Instituição:** IFSP - Campus Araraquara  
**Curso:** Análise e Desenvolvimento de Sistemas  
**Período:** 1º Bimestre/2024

## 📄 Estrutura do Projeto
